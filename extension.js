...
Your SOLE PURPOSE is to analyze the user's request and the current state, think step-by-step, and then generate a SINGLE JSON object to achieve the goal. Do not add any explanations or surrounding text.

**YOUR CORE LOGIC:**
1.  **Analyze Goal & State:** First, deeply understand the user's ultimate goal and review the provided 'current_state' to understand what has already been done (e.g., file listings, command outputs).
2.  **Think Step-by-Step:** Before making a plan, formulate an internal monologue (a "thought" process) to break down the problem. What is the most logical next step? Do I have enough information, or do I need to ask the user or explore the file system first?
...
Now, analyze the user's prompt and the current state, think like an expert, and generate the correct JSON response for the very next action.`;
...
class OllamaAgentViewProvider {
    ...
    async handlePrompt(prompt) {
        ...
        this.postMessageToWebview('addMessage', { role: 'user', text: prompt });
        this.postMessageToWebview('setStatus', { status: 'thinking' });
        this.postMessageToWebview('addMessage', { role: 'assistant', text: '' });

        const fullPrompt = AGENT_SYSTEM_PROMPT + prompt;
        const postData = JSON.stringify({ model: 'llama2', prompt: fullPrompt, stream: false, format: 'json' });
        const options = {
    ...
    async processAgentResponse(agentResponse) {
        ...
        switch (command) {
            case 'chat':
                this.postMessageToWebview('appendToMessage', { text: args.text });
                break;
            case 'ask_user':
                this.postMessageToWebview('appendToMessage', { text: args.question });
                break;
            case 'propose_plan':
